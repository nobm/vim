...
(function () {
  function error(href, mes) {
    console.error(`[VueImporter warn]: ${mes}\n---> ${href}`)
  }

  function warn(href, mes) {
    console.warn(`[VueImporter warn]: ${mes}\n---> ${href}`)
  }

  function compile(sfc, href) {
    let component = {}
    const parser = new DOMParser()
    const doc = parser.parseFromString("<body>" + sfc + "</body>", "text/html").body.children
  ...
  window.VueImporter = function (href) {
    ...
    xhr.onloadend = function (event) {
      if (this.status !== 200) {
        ...
        component = compile(this.responseText, href)
      }
    }
